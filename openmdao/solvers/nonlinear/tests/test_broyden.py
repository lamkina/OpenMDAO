...
class MixedEquation(om.ImplicitComponent):
    ...
    def setup(self):
        ...
        self.declare_partials(of="x12", wrt="x12", rows=np.arange(2), cols=np.arange(2), val=-np.array([3.0, 2]))
        self.declare_partials(of="x3", wrt="x3", rows=np.arange(1), cols=np.arange(1), val=-np.array([1.5]))
        self.declare_partials(of="x45", wrt="x45", rows=np.arange(2), cols=np.arange(2), val=-np.array([1, 0.5]))
...
class SpedicatoHuang(om.ImplicitComponent):
    ...
    def apply_nonlinear(self, inputs, outputs, residuals):
        ...
        residuals["y"][0] = y[0] + y[1] + x[0] + 0.25 * (y[1] - x[0]) ** 2
        residuals["y"][n - 1] = y[n - 1] + x[1] + y[n - 2] + 0.25 * (x[1] - y[n - 2]) ** 2
        for j in np.arange(1, n - 1):
            residuals["y"][j] = y[j] + y[j + 1] + y[j - 1] + 0.25 * (y[j + 1] - y[j - 1]) ** 2
    ...
    def linearize(self, inputs, outputs, jacobian):
        ...
        jacobian["y", "x"][0, 0] = 1.0 - 0.5 * (y[1] - x[0])
        jacobian["y", "y"][0, 0] = 1.0
        jacobian["y", "y"][0, 1] = 1.0 + 0.5 * (y[1] - x[0])

        jacobian["y", "x"][n - 1, 1] = 1.0 + 0.5 * (x[1] - y[n - 2])
        jacobian["y", "y"][n - 1, n - 1] = 1.0
        jacobian["y", "y"][n - 1, n - 2] = 1.0 - 0.5 * (x[1] - y[n - 2])

        for j in np.arange(1, n - 1):
            jacobian["y", "y"][j, j - 1] = 1.0 - 0.5 * (y[j + 1] - y[j - 1])
...
class TestBryoden(unittest.TestCase):
    ...
    def test_error_badname(self):
        ...
        model = prob.model = SellarStateConnection(
            nonlinear_solver=om.BroydenSolver(), linear_solver=om.LinearRunOnce()
        )
        ...
        msg = (
            "BroydenSolver in <model> <class SellarStateConnection>: The following variable names were not found: junk"
        )
        self.assertEqual(str(context.exception), msg)
    ...
    def test_simple_sellar(self):
        ...
        assert_near_equal(prob["state_eq.y2_command"], 12.05848819, 0.00001)

    def test_simple_sellar_cycle(self):
        # Test top level Sellar (i.e., not grouped).

        prob = om.Problem()
        model = prob.model = SellarDerivatives(nonlinear_solver=om.BroydenSolver(), linear_solver=om.LinearRunOnce())
    ...
    def test_sellar_state_connection_fd_system(self):
        ...
        assert_near_equal(prob.get_val("y1"), 25.58830273, 0.00001)
        assert_near_equal(prob.get_val("state_eq.y2_command"), 12.05848819, 0.00001)
...
class TestBryodenFeature(unittest.TestCase):
    def test_circuit(self):
        ...
        model.set_input_defaults("V", 0.0, units="V")
        ...
        assert_near_equal(p.get_val("circuit.R1.I") + p.get_val("circuit.D1.I"), 0.1, 1e-6)
