...
class SqliteRecorder(CaseRecorder):
    ...
    def _initialize_database(self):
        ...
        if MPI:
            ...
            if self._parallel and self._record_on_proc:
                ...
                print(
                    ...
                    "Cases from rank %d are being written to %s." % (rank, filepath)
                )
                if rank == 0:
                    ...
                    try:
                        ...
                        issue_warning(
                            "The existing case recorder metadata file, " f"{metadata_filepath}, is being overwritten.",
        ...
        if filepath:
            try:
                os.remove(filepath)
                issue_warning(
                    f"The existing case recorder file, {filepath}," " is being overwritten.", category=UserWarning
                )
            except OSError:
                pass
            ...
            if self._record_metadata:
                with self.metadata_connection as m:
                    ...
                    m.execute(
                        "INSERT INTO metadata(format_version, openmdao_version, abs2prom," " prom2abs) VALUES(?,?,?,?)",
                        (format_version, openmdao_version, None, None),
                    )
                    m.execute("CREATE TABLE driver_metadata(id TEXT PRIMARY KEY, " "model_viewer_data TEXT)")
                    m.execute(
    ...
    def startup(self, recording_requester):
        ...
        if isinstance(recording_requester, Driver):
            ...
            raise ValueError(
                "Driver encountered a recording_requester it cannot handle" ": {0}".format(recording_requester)
            )
        ...
        if self.connection:
            ...
            for v, abs_names in system._var_allprocs_prom2abs_list["input"].items():
                if v not in self._prom2abs["input"]:
                    ...
                    self._prom2abs["input"][v] = list(set(chain(self._prom2abs["input"][v], abs_names)))
            ...
            conns = zlib.compress(
                json.dumps(system._problem_meta["model_ref"]()._conn_global_abs_in2out).encode("ascii")
            )
            ...
            if self._record_metadata:
                with self.metadata_connection as m:
                    m.execute(
                        "UPDATE metadata SET "
                        + "abs2prom=?, prom2abs=?, abs2meta=?, var_settings=?, conns=?",  # nosec: trusted input
    ...
    def record_iteration_driver(self, driver, data, metadata):
        ...
        if not self._database_initialized:
            raise RuntimeError(
                f"{driver.msginfo} attempted to record iteration to "
    ...
    def record_iteration_problem(self, problem, data, metadata):
        ...
        if not self._database_initialized:
            raise RuntimeError(
                f"{problem.msginfo} attempted to record iteration to "
                f"'{self._filepath}', but database is not initialized;"
                " `run_model()`, `run_driver()`, or `final_setup()` "
                "must be called after adding a recorder."
            )
        ...
        if self.connection:
            ...
            if problem.recording_options["record_derivatives"] and driver._designvars and driver._responses:
                totals = data["totals"]
            else:
                totals = OrderedDict([])
    ...
    def record_iteration_system(self, system, data, metadata):
        ...
        if not self._database_initialized:
            raise RuntimeError(
                f"{system.msginfo} attempted to record iteration to "
                f"'{self._filepath}', but database is not initialized;"
                " `run_model()`, `run_driver()`, or `final_setup()` "
                "must be called after adding a recorder."
            )
    ...
    def record_iteration_solver(self, solver, data, metadata):
        ...
        if not self._database_initialized:
            raise RuntimeError(
                f"{solver.msginfo} attempted to record iteration to "
                f"'{self._filepath}', but database is not initialized;"
                " `run_model()`, `run_driver()`, or `final_setup()` "
                "must be called after adding a recorder."
            )
        ...
        if self.connection:
            ...
            with self.connection as c:
                ...
                if solver_type == "NL":
                    ...
                    raise RuntimeError(
                        "Solver type '%s' not recognized during recording. " "Expecting NL or LS" % solver.SOLVER
                    )
    ...
    def record_viewer_data(self, model_viewer_data, key="Driver"):
        ...
        if self._record_metadata and self.metadata_connection:
            ...
            try:
                with self.metadata_connection as m:
                    m.execute("INSERT INTO driver_metadata(id, model_viewer_data) VALUES(?,?)", (key, json_data))
        ...
        if self._record_metadata and self.metadata_connection:
            ...
            try:
                ...
                try:
                    ...
                    msg = (
                        f"Trying to record option '{key}' which cannot be pickled on this "
            ...
            with self.metadata_connection as m:
                m.execute(
                    "INSERT INTO system_metadata" "(id, scaling_factors, component_metadata) " "VALUES(?,?,?)",
                    (name, scaling_factors, pickled_metadata),
                )
    ...
    def record_metadata_solver(self, solver, run_number=None):
        ...
        if self._record_metadata and self.metadata_connection:
            ...
            with self.metadata_connection as m:
                m.execute(
                    "INSERT INTO solver_metadata(id, solver_options, solver_class)" " VALUES(?,?,?)",
                    (id, sqlite3.Binary(solver_options), solver_class),
                )
    ...
    def shutdown(self):
        ...
        if self._record_metadata and self.metadata_connection and self.metadata_connection != self.connection:
